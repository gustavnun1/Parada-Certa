...
public class ClientController {
    ...
    /** PUT /api/salvar — atualiza dados pessoais e endereço do cliente */
    @PutMapping("/salvar")
    public ResponseEntity<ApiResponse> salvar(@Valid @RequestBody ClienteUpdateRequest request) {
    ...
    private UserCompleteData buildUserData(Cliente cliente) {
        List<Veiculo> veiculos = veiculoRepository.findAllByClienteId(cliente.getId());
        Optional<Endereco> endereco = enderecoRepository.findByClienteId(cliente.getId());
        return new UserCompleteData(cliente, veiculos, endereco.orElse(null));
    }
}
