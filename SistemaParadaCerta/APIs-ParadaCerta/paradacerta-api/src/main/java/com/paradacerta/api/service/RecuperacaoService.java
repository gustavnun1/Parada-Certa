...
public class RecuperacaoService {
    ...
    public ApiResponse solicitarCodigo(SolicitarRecuperacaoRequest req) {
        ContaRecuperacao conta = buscarConta(req.getLogin(), req.getPerfil());

        String codigo = String.format("%06d", random.nextInt(1_000_000));
        LocalDateTime expiracao = LocalDateTime.now().plusMinutes(15);

        codigos.put(conta.chave(), new CodigoEntry(codigo, expiracao, conta.email()));

        try {
            enviarEmail(conta.email(), conta.nome(), codigo);
        ...
        String emailMascarado = mascararEmail(conta.email());
    ...
    public ApiResponse confirmarCodigo(ConfirmarRecuperacaoRequest req) {
        ...
        CodigoEntry entrada = codigos.get(conta.chave());
        if (entrada == null) {
        ...
        String hash = BCrypt.hashpw(req.getNovaSenha(), BCrypt.gensalt());
        if (conta.admin() != null) {
            conta.admin().setSenhaHash(hash);
            admRepository.save(conta.admin());
        } else {
            conta.cliente().setSenha(hash);
            clienteRepository.save(conta.cliente());
        }
        codigos.remove(conta.chave());
    ...
    private ContaRecuperacao buscarConta(String login, String perfil) {
        ...
        String normalizado = login == null ? "" : login.trim();
        boolean looksLikeCpf = normalizado.matches("\\d{11}");
        boolean admin = perfil != null && perfil.trim().equalsIgnoreCase("ADMIN");

        if (admin) {
            AdmEstacionamento adm = looksLikeCpf
                    ? admRepository.findByCpf(normalizado)
                            .orElseThrow(() -> new UsuarioNaoEncontradoException("Nenhuma conta encontrada com este CPF"))
                    : admRepository.findByEmailIgnoreCase(normalizado)
            ...
            if (Boolean.FALSE.equals(adm.getAtivo())) {
                throw new UsuarioNaoEncontradoException("Conta administrativa desativada");
            }
            if (adm.getEmail() == null || adm.getEmail().isBlank()) {
                throw new RequisicaoInvalidaException("Esta conta nao possui e-mail cadastrado para recuperacao");
            }
            return new ContaRecuperacao("ADMIN:" + adm.getId(), adm.getNomeCompleto(), adm.getEmail(), null, adm);
        }

        Cliente cliente = looksLikeCpf
                ? clienteRepository.findByCpf(normalizado)
                        .orElseThrow(() -> new UsuarioNaoEncontradoException("Nenhuma conta encontrada com este CPF"))
                : clienteRepository.findByEmail(normalizado)
                        .orElseThrow(() -> new UsuarioNaoEncontradoException("Nenhuma conta encontrada com este e-mail"));
        return new ContaRecuperacao("CLIENTE:" + cliente.getCpf(), cliente.getNome(), cliente.getEmail(), cliente, null);
    }
    ...
    private record ContaRecuperacao(
            String chave,
            String nome,
            String email,
            Cliente cliente,
            AdmEstacionamento admin
    ) {}
